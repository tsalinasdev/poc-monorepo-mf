...

  {
    ...
    settings: {
      ...
      'boundaries/elements': [
        ...
        // The federated entry pulls the remote's stylesheet in so its styles
        // travel with the contract; presentation is the only layer allowed near it.
        { type: 'assets', pattern: ['src/assets/**'] },
      ],
      // DI container and router are composition roots: they wire every layer
      // together by design, so they are exempt from the dependency rule.
      'boundaries/ignore': ['**/*.spec.ts', 'src/base/config/di/**', 'src/base/config/router/**'],
    },
    rules: {
