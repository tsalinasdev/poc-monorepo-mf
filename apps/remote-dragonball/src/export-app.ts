 ...
 * ALSO start with the basename, a navigation to `/dragon-ball` becomes
 * `/dragon-ball/dragon-ball` in the browser URL (the basename prefix is added
 * on top of the absolute route path). The routes must therefore be relative
 * to the basename.
 *
 ...
 * It is the section path (`/dragon-ball`) so the inner router can resolve
 * `/dragon-ball/1` to the detail screen while the host sees the same URL.
 */
function routesRelativeTo(basename: string | undefined) {
  if (!basename) return characterRoutes
  const prefix = basename.replace(/\/+$/, '')
  return characterRoutes.map((route) => ({
    ...route,
...
export default createBridgeComponent({
  ...
  appOptions: ({ app, basename }) => {
    ...
    const router = createRouter({
      ...
      routes: [{ path: '/', redirect: { name: 'character-list' } }, ...routesRelativeTo(basename)],
    })
