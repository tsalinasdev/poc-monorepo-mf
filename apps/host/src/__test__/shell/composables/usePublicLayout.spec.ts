 ...
 * route to START_LOCATION when the app unmounts, so reading the composable after
...
it('derives one nav item per route that opts in with meta.navLabel', async () => {
  const { result, app } = await withRouterAt(routes, '/alpha', usePublicLayout)
...
it('selects the section matching the current route', async () => {
  const { result, app } = await withRouterAt(routes, '/beta', usePublicLayout)
...
it('does not select a section whose path is a mere string prefix of another', async () => {
  ...
  const { result, app } = await withRouterAt(overlapping, '/alpha-beta', usePublicLayout)
